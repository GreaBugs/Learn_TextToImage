...
for epoch in range(n_epochs):
    ...
    for batch in train_dataloader:
        images = batch["images"].to(device)  # (32, 3, 64, 64)
        text = batch["text"]  # 32个文字描述
        

        # 使用 CLIP 模型编码文本
        text_inputs = tokenizer(text, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt")  # （32， 77）
        text_embeddings = text_encoder(text_inputs.input_ids.to(device)).last_hidden_state  # (32, 77, 512)

        timesteps = torch.randint(0, num_timesteps, (images.shape[0],), device=device).long() # 随机选择 timesteps  （32）
